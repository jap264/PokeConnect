...
if(!isset($_SESSION["userIn"])||$_SESSION["userIn"]!==true){
    ...
    $tid = $_POST["tid"];
		...
		if(empty($input_partyname)){
			$partyname_err = "  Please enter updated reply! <br> ";
		} else {
			...
			if(empty($partyname_err)){
				$sql = "UPDATE threadchat SET mtext=? WHERE threadchat_ID=?";
          ...
          $param_p_id = $p_id;
					...
					if(mysqli_stmt_execute($stmt)){
						header("location: viewthread.php?id=$tid");
						exit();
					} else{
		...
		if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
			$p_id = trim($_GET["id"]);
			$sql = "SELECT * FROM threadchat WHERE threadchat_ID=?";
			if($stmt = mysqli_prepare($link, $sql)){
		...
		<section id="main" class="wrapper"  style="padding-top:20px; padding-bottom:0px;">
			<div class="container"  style="width: 95% !important;">
					<header class="major special">
						<h2 style="margin-bottom: 6px !important;"><img src="images/party.png" height=60px>  Edit Reply</h2>
						<a href="dashboard.php" class="button alt" style="float:right;">Back to thread</a>
					...
					<h3>Enter the Fields</h3>
								<div class="row uniform 50%">
									...
									<div class="12u 12u$(xsmall)">
										<h6>Your Reply to the thread</h6>
										<input type="text" name="partyname" id="partyname" value="<?php echo $partyname; ?>" placeholder="reply "/>
...
<input type="hidden" name="tid" value="<?php echo trim($_GET['tid']); ?>"/>
									<div class="12u$">
										<ul class="actions">
											...
											<li style="float:right;"><input type="submit" value="Update Reply" class="special" /></li>
										</ul>
									</div>
								</form>
